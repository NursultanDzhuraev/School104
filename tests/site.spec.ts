...
test("root redirects and unknown locale / news return 404", async ({
  page,
  request,
}) => {
  await page.goto("/");
...
for (const locale of ["kg", "ru"] as const) {
  test(
    locale + " pages, metadata, assets and internal links",
    async ({ page, request }) => {
      const errors: string[] = [];
      ...
      const paths = [...sections, ...news.map((item) => "/news/" + item.slug)];
      ...
      for (const href of targets) {
        ...
        if (url.hash) {
          ...
          expect(
            await page
              .locator('[id="' + decodeURIComponent(url.hash.slice(1)) + '"]')
              .count(),
            href,
          ).toBeGreaterThan(0);
  ...
  for (const width of [1920, 1440, 1024, 768, 430, 375]) {
    test(locale + " responsive " + width, async ({ page }) => {
      ...
      await page.screenshot({
        path: "test-results/" + locale + "-home-" + width + ".png",
        fullPage: true,
      });
    });
  }
}
test("news filters, empty search, pagination and article locale switch", async ({
  page,
}) => {
  ...
  await page
    .getByRole("searchbox", { name: "Издөө", exact: true })
    .fill("zzzz-does-not-exist");
...
test("mobile menu, search, keyboard gallery and schedule", async ({ page }) => {
  ...
  await page
    .getByRole("button", { name: "Сайттан издөө", exact: true })
    .click();
  await page.getByRole("dialog").getByRole("textbox").fill("Окуучуларга");
  await page
    .getByRole("dialog")
    .getByRole("link", { name: "Окуучуларга", exact: true })
    .click();
...
test("PDF filters and all files have real PDF signatures", async ({
  page,
  request,
}) => {
  ...
  await page
    .getByRole("link", { name: "Скачать: Устав школы", exact: true })
    .click();
  expect((await downloadPromise).suggestedFilename()).toBe(
    "sample-charter.pdf",
  );
});
test("contact form validates and never sends visitor data", async ({
  page,
}) => {
  ...
  await page
    .getByLabel("Сообщение", { exact: false })
    .fill("Тестовое сообщение");
  await page.getByRole("button", { name: "Отправить", exact: true }).click();
