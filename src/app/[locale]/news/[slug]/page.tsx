...
export async function generateStaticParams() {
  return (await content.getNews()).map((item) => ({ slug: item.slug }));
}
export async function generateMetadata({ params }: Props) {
  const { locale: raw, slug } = await params;
  const locale = requireLocale(raw);
  const item = await content.getNewsBySlug(slug);
  if (!item) notFound();
  ...
  return {
    ...base,
    openGraph: { ...base.openGraph, type: "article", publishedTime: item.date },
  };
}
export default async function Article({ params }: Props) {
  ...
  const item = await content.getNewsBySlug(slug);
  if (!item) notFound();
  const related = (await content.getNews())
    .filter((other) => other.id !== item.id)
    .slice(0, 3);
  return (
    <>
      <article className="section article-container">
        <Link className="text-link" href={"/" + locale + "/news"}>
        ...
        <div className="article-copy">
          {item.demo && (
            <p className="sample-note">
              {t.news.articleSample} {t.common.photoNote}
            </p>
          )}
          <p className="article-lead">{item.description[locale]}</p>
      ...
      <section className="section muted">
        <div className="container">
          ...
          <div className="news-grid home-news">
            {related.map((other) => (
              <NewsCard key={other.id} item={other} locale={locale} />
            ))}
          </div>
        </div>
      </section>
