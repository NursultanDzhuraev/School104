...
export default async function Documents({ params }: Props) {
  const locale = requireLocale((await params).locale);
  const t = messages(locale);
  const documents = await content.getDocuments();
  return (
    <>
      <PageHero
        locale={locale}
      ...
      <section className="section">
        <div className="container">
          {documents.some((item) => item.demo) && (
            <p className="sample-note">{t.documents.note}</p>
          )}
          <DocumentExplorer items={documents} locale={locale} />
        </div>
      </section>
    </>
  );
