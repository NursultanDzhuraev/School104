  ...
  return (
    <>
      <div className="filter-toolbar">
        <div className="filter-list" aria-label={t.news.title}>
          <button
          ...
          {(Object.keys(t.news.categories) as NewsCategory[]).map((key) => (
            <button
              key={key}
      ...
      {category === "all" && !query && current === 1 && items[0] && (
        <div className="featured-wrap">
          <p className="eyebrow">{t.news.featured}</p>
          <NewsCard item={items[0]} locale={locale} featured />
        </div>
      )}
      <div className="news-layout">
            ...
            <nav className="pagination" aria-label={t.common.page}>
              ...
              {Array.from({ length: pages }, (_, i) => i + 1).map((n) => (
                <button
                  key={n}
                  className={
                    n === current ? "page-button active" : "page-button"
                  }
                  aria-current={n === current ? "page" : undefined}
                  aria-label={t.common.page + " " + n}
                  onClick={() => setPage(n)}
                >
                  {n}
                </button>
        ...
        <aside className="news-sidebar">
          ...
          <Announcements locale={locale} items={announcements} compact />
        </aside>
      </div>
    </>
  );
